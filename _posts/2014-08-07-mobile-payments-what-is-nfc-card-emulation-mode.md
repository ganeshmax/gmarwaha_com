...
An [NFC]({{ site.baseurl }}/2014/08/03/mobile-payments-what-is-nfc.html) enabled device can operate in three different modes – `reader/writer` mode, `peer-to-peer` mode and the all important `card-emulation` mode.


In **Reader/Writer mode**, an NFC device behaves as a reader for NFC tags, such as the [contactless smart cards]({{ site.baseurl }}/2014/05/11/mobile-payments-what-is-a-contactless-chip-card.html) and RFID tags. It detects a tag immediately in close proximity by using collision avoidance mechanism. Once detected, it can either read data from or write data to the detected tag. Smart posters are an important application for this mode.
